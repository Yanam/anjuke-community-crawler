...
def addInfo(info):
    ...
    try:
        c.execute(
            r'''INSERT INTO `prices` (`fang_id`,`xiaoqu_url`,`xiaoqu_name`,`city`,`area`,`price`,`huxing`,`qianyue`) VALUES(%s,%s,%s,%s,%s,%s,%s,%s);''',
            (info['fang_id'], info['xiaoqu_url'], info['xiaoqu_name'], info['city'], info['area'], info['price'],
             info['huxing'], info['qianyue']))
        result = True
    except mysql.IntegrityError:
        c.execute(
            r'''UPDATE `prices` SET `xiaoqu_url`=%s,`xiaoqu_name`=%s,`city`=%s,`area`=%s,`price`=%s, `huxing`=%s, `qianyue`=%s WHERE `fang_id`=%s;''',
        ...
        if r.status_code == 200 or r.status_code == 304:
            ...
            if re.search(r'''5i5j.com/xiaoqu/(\d+)\.html''', url) != None:
                ...
                for ele in d('ul.yizucontent').items():
                    fang_id = ele.find('li').eq(0).find('a').attr('href')
                    ...
                    if insert_s:
                        LazyFW.log(
                            "Update Data: fang_id: %s, area: %s" % (data['fang_id'], data['area'],))
